...
void goToDeepSleep() {
  if ((counter == 16) && (displayMode == "horizontal 4") && (deepSleepEnable)) {
  ...
  if ((counter == 22) && (displayMode == "horizontal 6") && (deepSleepEnable)) {
    Serial.println("Go to Deep Sleep");
    epd_poweron();
    epd_draw_image(sleepIcon, (uint8_t *)sleep_data, BLACK_ON_WHITE);
    epd_poweroff_all();
...
void errorMessage() {
  ...
  if ((millis() >= waitTimeCounterAction) && (!firstStartTime) && (counter != 22) && (displayMode == "horizontal 6")) {
    error = true;
  }
  if ((millis() >= waitTimeCounterAction) && (!firstStartTime) && (counter != 16) && (displayMode == "horizontal 4")) {
    error = true;
  }
  ...
  if (error) {  //error message ERROR_WIFI was set in wifi() function
    ...
    if ((counter != 16) && ((WiFi.status() == WL_CONNECTED)) && (displayMode == "horizontal 4") && (paperScreenOnline) && (ERROR_MESSAGE != ERROR_TIME_TO_SLEEP)) {
      ERROR_MESSAGE = ERROR_COUNTER;
    }

    if ((counter != 22) && ((WiFi.status() == WL_CONNECTED)) && (displayMode == "horizontal 6") && (paperScreenOnline) && (ERROR_MESSAGE != ERROR_TIME_TO_SLEEP)) {
      ERROR_MESSAGE = ERROR_COUNTER;
    }
